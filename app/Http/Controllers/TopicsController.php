        ...
        if (!empty($WebmasterSection)) {
            ...
            if (@Auth::user()->permissionsGroup->view_status) {
                $Topics = Topic::where('created_by', '=', Auth::user()->id)->where('webmaster_id', '=',
                    $webmasterId)->orderby('date','desc')->orderby('row_no',
                    'desc')->paginate(env('BACKEND_PAGINATION'));
            } else {
                $Topics = Topic::where('webmaster_id', '=', $webmasterId)->orderby('date','desc')->orderby('row_no',
                    'desc')->paginate(env('BACKEND_PAGINATION'));
            }
    ...
    public function store(Request $request, $webmasterId)
    {
        ...
        if (!empty($WebmasterSection)) {
            ...
            $this->validate($request, [
                'photo_file' => 'mimes:png,jpeg,jpg,gif|max:9000',
            ...
            if ($WebmasterSection->type == 4){
                $slugs = Helper::URLSlug($request->title_vi.'-'.$request->sapo, $request->title_en.'-'.$request->sapo, "topic", 0);
            }else{
